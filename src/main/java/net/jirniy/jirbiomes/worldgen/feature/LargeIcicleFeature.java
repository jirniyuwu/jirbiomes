...
// copied from large dripstone feature and just replaced dripstone with mod blocks
public class LargeIcicleFeature extends Feature<LargeDripstoneConfiguration>  {
    public LargeIcicleFeature(final Codec<LargeDripstoneConfiguration> codec) {
        super(codec);
    }
    ...
    public boolean place(final FeaturePlaceContext<LargeDripstoneConfiguration> context) {
        ...
        if (!isEmptyOrWater(level, origin)) {
            ...
            Optional<Column> column = Column.scan(level, origin, config.floorToCeilingSearchRange, SpeleothemUtils::isEmptyOrWater, (state) -> SpeleothemUtils.isBaseOrLava(state, ModBlocks.PERMAFROST_BLOCK, config.replaceableBlocks));
            if (!column.isEmpty()) {
    ...
    private void placeDebugMarkers(final WorldGenLevel level, final BlockPos origin, final Column.Range range, final WindOffsetter wind) {
        ...
        for(BlockPos.MutableBlockPos pos = origin.atY(range.floor() + 2).mutable(); pos.getY() < range.ceiling() - 1; pos.move(Direction.UP)) {
            ...
            if (isEmptyOrWater(level, windAdjustedPos) || level.getBlockState(windAdjustedPos).is(ModBlocks.PERMAFROST_BLOCK)) {
    ...
    private static final class LargeDripstone {
        ...
        private int getHeightAtRadius(final float checkRadius) {
            return (int)getSpeleothemHeight((double)checkRadius, (double)this.radius, this.scale, this.bluntness);
        }
                    ...
                    if (!(currentRadius > (float)this.radius)) {
                        ...
                        if (height > 0) {
                            ...
                            for(int i = 0; i < height && pos.getY() < maxY; ++i) {
                                BlockPos windAdjustedPos = wind.offset(pos);
                                if (isEmptyOrWaterOrLava(level, windAdjustedPos)) {
                                    hasBeenOutOfStone = true;
                                    Block block = SharedConstants.DEBUG_LARGE_DRIPSTONE ? Blocks.GLASS : ModBlocks.PERMAFROST_BLOCK;
